...
def main():
    ...
    for name in names:
        ...
        friend_stats[name] = {'name': name, 'top_artist': top_artist, 'top_album': top_album, 'top_track': top_track, 'artists_listened': artists_listened, 'albums_listened': albums_listened, 'tracks_listened': tracks_listened, 'total_scrobbles': total_scrobbles}
...
def get_scrobble_count(url, user, key):
    ...
    scrobbles = requests.get(url + '?method=user.getinfo&user=' + user + '&api_key=' + key + '&format=json').json()
